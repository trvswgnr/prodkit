...
**Success-gated release (`.withRelease`).** `withCleanupCoreOp` in `packages/op/src/core/fluent-nullary.ts`
...
`.withRelease`; LIFO ordering applies across all of them.
...
**`.withRelease` is not a synonym for defer.** Running release on typed failure would convert
...
**Make `.withRelease` run on failure like defer.** Rejected: release hooks often assume a successful
...
- Do not move effectful or suspending cleanup into bare generator `finally` blocks; use
  `Op.defer`, `.on("exit")`, or `.withRelease` as appropriate.
- Do not merge `closeGenerator` fault handling with `runFinalizersSafely`; the asymmetry is the
  contract.
- `DESIGN.md` documents LIFO ordering, finalizer precedence, and `withRelease` success-gating as
