...
- `@prodkit/std` is a general runtime-agnostic utility layer (typed helpers for platform gaps, prefer-native, zero runtime deps, tree-shakeable subpaths such as `@prodkit/std/array`); it is not the home for op-specific features. DI moves to `@prodkit/op/di`.
