...
- Op-native modules (runtime-agnostic, no third-party deps beyond `better-result`) ship as `@prodkit/op` subpath exports (for example `@prodkit/op/di`, `@prodkit/op/policy`); do not split them into separate npm packages. `@prodkit/std` is being retired after DI moves to `@prodkit/op/di`.
