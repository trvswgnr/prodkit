...
- Workspace taxonomy: `packages/*` (publishable), `packages/shared` (`@prodkit/shared`, private workspace types/config), `examples` (`@prodkit/examples`, consumer smoke workspace), `benchmarks/*` (performance harnesses), `tools` (`@prodkit/tools`, maintainer scripts), and `apps/*` reserved for runnable product/demo apps.
