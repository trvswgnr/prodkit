...
- `@prodkit/op/hkt` owns the reusable HKT primitives (`HKT`, `HKTArg`, `Apply`, `HKT_ARGS`, `HKT_RESULT`); policy consumes them internally and does not re-export them from `@prodkit/op/policy`.
